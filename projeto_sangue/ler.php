...
tudo depende de você!</p>
		...
		<div id="informaçao">
        <?php 
		$id_depoimento = capGet('id_depoimento');
		$selecao="SELECT * FROM depoimentos WHERE id_depoimento='$id_depoimento'";
		$dados = mysql_query($selecao);
	
		while($linha=mysql_fetch_array($dados)){
			echo"<h2 class='titu2'>Autor:".strtoupper($linha ['nome'])."</h2>
			<p class='#'>Titulo:".$linha ['titulo']."</p>
